...
function countApplesAndOranges(s, t, a, b, apples, oranges) {
    ...
    const getCount = (arr, distance) => {
        let count = 0;
        for(let i = 0; i < arr.length; i++){
            let disFromTree = distance + arr[i];
            disFromTree >= s && disFromTree <= t ? count++ : count
        }
        return count
    }
    console.log(getCount(apples, a));
    console.log(getCount(oranges, b));
}
